...
void window::dessinerRectangle(int hauteur, int largeur, SDL_Rect position, std::string couleur)
{
	...
	{
	case 'n': //noir
		pixel = SDL_MapRGB(surface->format, 60, 60, 60);
		break;
	case 'r'://rouge
		pixel = SDL_MapRGB(surface->format, 236, 60, 34);
		break;
	case 'b'://bleu
		pixel = SDL_MapRGB(surface->format, 81, 73, 222);
		break;
	case 'v'://vert
		pixel = SDL_MapRGB(surface->format, 84, 175, 39);
		break;
	default:
...
void window::ajouterText(int taille, SDL_Rect position, std::string texte_a_afficher){
    ...
    if(texte_a_afficher.size()>9 ){ //pour permettre un retour a la ligne si la phrase a afficher est trop longue
            int i=1;
